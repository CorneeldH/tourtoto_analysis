...
-   [x] Basis gedaan
-   [ ] Mapping table maken voor punten eind
-   [ ] Omklappen stage_results + mapping table voor punten
-   [ ] In manipulate riders horse factor berekenen, if gekozen --\> grey / etc / etc, zie mapping_DEM_leeftijd_cat
-   [ ] Daarna if grey \* 2 etc etc
...
```
   ...
   fun2(square())
   fun2 -- Inherits from --> global
   call((square(10)))
   call -- Execution Environment --> callEnv((square(10)))
   callEnv -- Looks up 'x' in --> callEnv
   callEnv -- Looks up 'exp' in --> fun2
